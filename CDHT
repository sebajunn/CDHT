...
def init(argv):
  ...
  pred1 = -1; #predecessors will be set later based on incoming ping signals
  pred2 = -1;
...
def main(screen):
  ...
  curses.init_pair(3, curses.COLOR_CYAN, COLOR_DEFAULT);
  curses.init_pair(4, curses.COLOR_RED, COLOR_DEFAULT);
  curses.init_pair(5, curses.COLOR_GREEN, COLOR_DEFAULT);
  ...
  consolePrint (screen, 0, "Attempting to join the CDHT network as Peer (" + makeColComp(5, str(myPeer)) + ")...");
  consolePrint (screen, 0, "Successfully joined CDHT network."); #simulate fake join message because it looks nice!
  consolePrint (screen, 0, "Welcome to this CDHT network!");
  consolePrint (screen, 0, "Enter valid commands at the bottom of this terminal screen. Command " + makeColComp(3, "quit") + " will exit the application.");

  # Display warning message if screen width size is too small
  height, width = screen.getmaxyx();
  if (width < MIN_REC_WIDTH):
    consolePrint (screen, 6, makeColComp(4, "A minimum terminal width of " + str(MIN_REC_WIDTH) + " characters is recommended (current: " + str(width) + ")."));
  ...
  while True:
      ...
      if s == "quit":
        ...
        sendChurnMessage(4, succ1, succ2, myPeer, LOCALHOST, peerToPort(pred1));
        sendChurnMessage(4, succ1, succ2, myPeer, LOCALHOST, peerToPort(pred2));

        terminate_thread(tPingMonitor); #kill threads safely
        terminate_thread(tTCPMonitor);

        consolePrint (screen, 0, "Leaving CDHT network and terminating program. Please wait for running threads to terminate."); #quit message
        ...
        try:
          ...
          consolePrint (screen, 0, "Invalid command parameters were provided. Provided command was: " + s);
          continue;
        ...
        try:
          ...
          consolePrint (screen, 0, "Invalid file was requested. File name must be a 4 length numeral.");
          continue;
        ...
        if fileStatus == 0:
          ...
          sendFTMessage(reqFileHashNum, 0, myPeer, LOCALHOST, peerToPort(succ1));
        elif fileStatus == 1:
          #File is stored locally
          consolePrint (screen, 4,   "File " + makeColComp(4, reqFileHash) + " is stored locally.");
          continue;
        elif fileStatus == 2:
          # The next peer has the file, send a special message
          sendFTMessage(reqFileHashNum, 2, myPeer, LOCALHOST, peerToPort(succ1));

        # Display file request sent message
        consolePrint (screen, 3,   "File request message for " + makeColComp(4, reqFileHash) + " has been sent to successor Peer (" + makeColComp(5, str(succ1)) + ").");
        ...
        if command == "off":
          consolePrint (screen, 0, "Ping messages have been disabled.");
          showPingMessages = False;
        elif command == "on":
          consolePrint (screen, 0, "Ping messages have been enabled.");
          showPingMessages = True;
        else:
          consolePrint (screen, 0, "Invalid command parameters were provided. Provided command was: " + s);
      ...
      # Unknown command
        ...
        consolePrint (screen, 0, "Invalid command '" + s + "' provided.");
...
def pingMonitor(screen, Ping):
  ...
  while True:
    ...
    if (time.time() - lastPingsSent) > PINGSEND_FREQUENCY:
      ...
      if succ1 != -2:
        sendPing(0, sequenceNum, myPeer, LOCALHOST, peerToPort(succ1));
      if succ2 != -2:
        sendPing(0, sequenceNum, myPeer, LOCALHOST, peerToPort(succ2));
    ...
    if succ1JustDied and succ1 != -2:
      succ1LastAck = sequenceNum;
      succ1JustDied = False;

    if succ2JustDied and succ2 != -2:
    ...
    if succ1 != -2 and succ1NumMissedAcks >= PING_MISSED_ACK_DEAD_NUM:

      consolePrint(screen, 5, "Peer (" + makeColComp(5, str(succ1)) + ") is no longer alive.")
      ...
      sendChurnMessage(5, 0, 0, myPeer, LOCALHOST, peerToPort(succ2));

    if succ2 != -2 and succ2NumMissedAcks >= PING_MISSED_ACK_DEAD_NUM:

      consolePrint(screen, 5, "Peer (" + makeColComp(5, str(succ2)) + ") is no longer alive.")
      ...
      sendChurnMessage(5, 0, 0, myPeer, LOCALHOST, peerToPort(succ1));
    ...
    try:
      ...
      if msgType == 0:
        consolePrint(screen, 1, "A ping request message was received from Peer (" + makeColComp(5, str(senderPeerID)) + ")");
        ...
        if (pred1 != -1 and pred2 != -1) and (senderPeerID != pred1 and senderPeerID != pred2):
          pred1 = -1;
          pred2 = -1;
        ...
        sendPing(1, recSeq, myPeer, LOCALHOST, peerToPort(senderPeerID));
        ...
        consolePrint(screen, 2 , "A ping response message was received from Peer (" + makeColComp(5, str(senderPeerID))+ ")")
    except socket.error:
      pass;
    ...
    try:
      ...
      while True:
        ...
        if msgType == 4:
          ...
          #Print churn message
          consolePrint(screen, 5, "Peer (" + makeColComp(5, str(senderPeerID)) + ") will depart from the network.");
          consolePrint(screen, 5, "My first successor is now Peer (" + makeColComp(5, str(succ1))  + ").");
          consolePrint(screen, 5, "My second successor is now Peer (" + makeColComp(5, str(succ2))  + ").");

        # A peer has asked for successor information        
        elif msgType == 5:
          #Send response message to sender
          sendChurnMessage(6, succ1, succ2, myPeer, LOCALHOST, peerToPort(senderPeerID));
        ...
        # A peer has responded with query information
          ...
          if succ1 == -2:
            ...
            if nextPeerSucc1 == lastDeadPeer or nextPeerSucc1 == -2:
              succ2 = nextPeerSucc2;
          ...
          if msgType == 2:
            ...
            sendFTMessage(filehash, 3, myPeer, LOCALHOST, peerToPort(senderPeerID));
            consolePrint(screen, 4, "File " + makeColComp(4, str(filehash).zfill(4)) + " is stored here. A response message has been sent to Peer (" + makeColComp(5, str(senderPeerID))  + ").");

          #We received a response for a requested file request
          elif msgType == 3:
            consolePrint(screen, 4, "Received a response message from Peer (" + makeColComp(5, str(senderPeerID))  + "), which has the file " + makeColComp(4, str(filehash).zfill(4)) + ".");
          ...
          #Else perform regular processing
            ...
            if fileStatus == 0:
              ...
              sendFTMessage(filehash, 1, senderPeerID, LOCALHOST, peerToPort(succ1));
              ...
              # Directory contact sender with response
              sendFTMessage(filehash, 3, myPeer, LOCALHOST, peerToPort(senderPeerID));
              consolePrint(screen, 4, "File " + makeColComp(4, str(filehash).zfill(4)) + " is stored here. A response message has been sent to Peer (" + makeColComp(5, str(succ1))  + ").");
...
def consolePrintLine (screen, pos, control, message):
  ...
  if control == 6:
    screen.addstr(pos, 0, "[WARNING]", curses.color_pair(2));
  elif control == 3:
    screen.addstr(pos, 0, "[FILE REQ]", curses.color_pair(6));
  elif control == 4:
    screen.addstr(pos, 0, "[FILE RES]", curses.color_pair(6));
  elif control == 5:
    screen.addstr(pos, 0, "[PEER CRN]", curses.color_pair(7));
  elif control == 2:
    screen.addstr(pos, 0, "[PING REQ]", curses.color_pair(1));
  elif control == 2:
    screen.addstr(pos, 0, "[PING RES]", curses.color_pair(1));
  elif control == 0:
    screen.addstr(pos, 0, "[STATUS]", curses.color_pair(1));
...
def consolePrint (screen, control, message):
  ...
  if (not showPingMessages) and (control == 1 or control == 2):
    return;
