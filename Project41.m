...
for i=1:L             
   ...
   % size(zz(1));
    ...
    if flag
            ...
            if (flag1 == 0)
                rollInput = unbiasedAttitude(times<laserTimes(i),1)*k;
                set(hRoll,'xdata',times(times<laserTimes(i)),'ydata',rollInput);
                set(hRoll2,'xdata',laserTimes(1:i),'ydata',myRollData(1:i));
                pitchInput = unbiasedAttitude(times<laserTimes(i),2)*k;
                set(hPitch,'xdata',times(times<laserTimes(i)),'ydata',pitchInput);
                set(hPitch2,'xdata',laserTimes(1:i),'ydata',myPitchData(1:i));
                yawInput = unbiasedAttitude(times<laserTimes(i),3)*k;      
                set(hYaw,'xdata',times(times<laserTimes(i)),'ydata',yawInput);
                if (flag1 == 0)
