...
for i=1:L             
    ...
    if(rsquare < quality && rsquare > 0)
     % solving equations analytically
            ...
            trMatrix = [cos(myThetaY)*cos(myThetaZ) -cos(myThetaX)*sin(myThetaZ)+sin(myThetaX)*sin(myThetaY)*cos(myThetaZ)  sin(myThetaX)*sin(myThetaZ)+cos(myThetaX)*sin(myThetaY)*cos(myThetaZ);
...
function indexOfRay = PopulateRay(rays)
     global MyContext
    flag1 = 0;
    flag2 = 0;
    flag3 = 0;  
