...
public class cdht {
    ...
    /** Time <-> pings  */
    public static final int PingScoreTimer = 3*10000;
        ...
        public void run() {
            ...
            try {
                ...
                if (LostPingIntial >= maxLostPingScore || LostPingSecondary >= maxLostPingScore) {
                    ...
                    int peerLive;
                    peerLive = secondPeer;
                    if (maxLostPingScore <= LostPingSecondary) {
                        peerDead = secondPeer;
                        peerLive = peerFirst;
                    }
                    System.out.printf("Peer %d is no longer alive.%n", peerDead);
            ...
            do {
                if (!"quit".equalsIgnoreCase(myStringInput)) {
                    if (myStringInput.length() != 12 || !"request".equalsIgnoreCase(myStringInput.substring(0, 7))) {
                        System.out.printf("Unknown command : %myScanner%n", myStringInput);
                    ...
                    } else {
                        {
                            ...
                            try {
                                ...
                                String mySentence;
                                mySentence = String.format("REQUEST %myScanner %d %d", myRequestedFile, peerIdentification, peerIdentification);
                                outputStreamToServer.writeBytes(String.format("%myScanner\n", mySentence));
                                socketSend.close();
                                System.out.printf("File request message for %myScanner has been sent to my successor. File myHash is %d.%n", myRequestedFile, myHash);
                    ...
                    myExitMessage = String.format("QUIT %d %d %d", predecessorFirst, secondPredecessor, peerIdentification);
                    ...
                    myExitMessage = String.format("QUIT %d %d %d", peerFirst, secondPeer, peerIdentification);
                    outputToServer.writeBytes(String.format("%myScanner\n", myExitMessage));
                    ...
                    if (secondPredecessor != secondPeer) {
                        ...
                        outputServerB.writeBytes(String.format("%myScanner\n", myExitMessage));
                        BufferedReader inputFromB;
                        inputFromB = new BufferedReader(new InputStreamReader(SocketSendB.getInputStream()));
                        inputFromB.readLine();
                        SocketSendB.close();
                    }
                    System.out.printf("Peer %d has left the network.%n", peerIdentification);
                    System.exit(0);
